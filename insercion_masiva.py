...
INSTITUTIONS = {
    "AR": ["UBA", "UTN", "ITBA", "UADE"],
    "US": ["MIT", "Stanford", "Harvard"],
    "UK": ["Oxford", "Cambridge", "UCL"],
    "DE": ["TUM", "LMU", "TU Berlin"]
}
...
def create_document():
    ...
    doc = {
        ...
        "student_id": f"STU-{random.randint(1, 500000)}",
        "country": country,
        "institution": random.choice(INSTITUTIONS[country]),
        "subject": random.choice(SUBJECTS),
        "original_grade": generate_grade(country),
        "metadata": {
...
async def main():
    ...
    print("🚀 Starting 1M insert...")

    while inserted < TOTAL_RECORDS:

        batch = [create_document() for _ in range(BATCH_SIZE)]
